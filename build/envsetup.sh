...
- pixelgerrit:     A Git wrapper that fetches/pushes patch from/to morbios Gerrit Review.
...
- githubremote:    Add git remote for morbios Github.
...
- repopick:        Utility to fetch changes from morbios Gerrit.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/MORBIOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop org.morbios.device | grep -q "$CUSTOM_BUILD"); then
...
function githubremote()
{
    ...
    git remote add github https://github.com/Morbi-OS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop org.morbios.device | grep -q "$CUSTOM_BUILD");
    then
...
function pixelgerrit() {
    ...
    local user=`git config --get review.gerrit.morbios.org.username`
...
function pixelrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "morbios Gerrit Rebase Usage: "
    ...
    git fetch "http://gerrit.morbios.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop org.morbios.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
